		...
		<div class="row" style="margin-bottom: 0px">
			<div class="col s12 ">
				<div class="card hoverable">
					<div class="card-image">
						<img src="<?php echo base_url('assets/images/background_card.jpg') ?>">
						<div class="card-title" style="text-shadow: 1px 1px 1px black; font-size:calc(1em + 1vw);"><i class="material-icons " style="vertical-align: -3px;">assignment_turned_in</i> <strong>Crear tabla de aplicabilidad</strong></div>
					</div>
					<div class="card-content section">
						<form action="" name = "data_so" id = "datos_so">
							<div class="row ">
						...
						<div class="row">
							...
							<div class="input-field col s12 m8 l8 ">
								...
								<select name="articulo" id="articulo">
								 ...
								 <label>Determine el tipo de obligaciones a generar</label>
							</div>
					</form>
				</div>
				<div class="row center">
					<div class="input-field col s12 m12 l12 ">
						<button  id="load_frac" class="btn waves-effect waves-light "  ><i class="material-icons"style="vertical-align: -4px">file_upload</i> Crear tabla y Cargar Fracciones</button>
					</div>
				</div>
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		$('#detail_frac').empty();
		$("#info_table").modal("open");


	});
	$("body").on('change', "#articulo", function () {
		$('#load_frac').attr('disabled', false);
	});
	...
	$("body").on('click', "#load_frac", function (event) {
		event.preventDefault();
		base_url = "<?php echo base_url(); ?>";
		$('#load_frac').attr('disabled', true);
		$('#detail_frac').empty();
		URL = base_url + "tablas_ap/select_fraccion_aplic_so";
		$.ajax({
			async: true,
			type: "POST",
			url: URL,
			cache: false,
			data: $('#datos_so').serialize(),
			dataType: "html",
			beforeSend: function () {
				$('#pre-saving').show();
				$('#detail_frac').empty();
