...
<form action="" name = "data_art_74" id="data_art_74">
	...
	<?php foreach ($data_fraccion as $data_frac): ?>
		...
		<tr>
			<td style="font-size:12px ">Art. <?php
					...
					<div class="col s12 section center">
						...
						if ($data_frac->fundamentacion_ltg !== "") {
							...
							<div class="switch">   
								<label>
									NO
									<input disabled class="aplica"  type="checkbox" checked=""  id="<?php echo $data_frac->id_fraccion ?>"value="1">
							...
							<div class="switch">   
								<label>
									NO
									<input  class="aplica"  type="checkbox" checked=""  id="<?php echo $data_frac->id_fraccion ?>"value="1">
									<span class="lever"></span>
									SI
								</label>
							</div>
			...
			<td style="font-size:12px ">
				<div class="row">
					...
					<?php } else { ?>
						<div style="display:none" id = "just<?php echo$data_frac->articulo . $data_frac->id_fraccion ?>">
							...
							<div class="input-field col s10">
								<textarea disabled="" name ="fundamenta_frac" id="<?php echo$data_frac->articulo . $data_frac->id_fraccion ?>" class="materialize-textarea justifica_so validate"></textarea>
								<label class="active">Ingrese Justificación </label>
								<span class="helper-text" data-error="wrong" data-success="Justificación guardada correctamente">Fundamentación de No Aplicabilidad.</span>

							</div>
							<div class="input-field col s2" >
								<a  disabled class=' btn btn-floating btn-small  waves-effect waves-light save_just_esp' href="#!" data-value="<?php echo $data_frac->id_fraccion ?>" id="save<?php echo $data_frac->articulo . $data_frac->id_fraccion ?>"><i class="material-icons">save</i></a>
							</div>
...
<div class="row center section">
	<?php
	if($enabled_preview==1){
		$activate = '';
	}else{
		$activate = 'disabled';
	} ?>
	<a class="btn waves-effect waves-light "id ="finalizar" href="javascript:getURL()" <?php echo $activate?> ><i class="material-icons" style="vertical-align:-4px">pageview</i> Ver vista previa de  tabla </a>
</div>
...
<script type="text/javascript">
	...
	$(document).ready(function () {
		...
		$("body").on('click', ".save_just_esp", function (event) {
			...
			if (!this.checked) {
				...
				$.ajax({
					async: true,
					type: "POST",
					url: base_url + "tablas_ap/save_funs_so_esp",
					cache: false,
					data: {'id_frac': id_fraccion, 'ejercicio': ejercicio, 'id_cat_so': id_cat_so, 'id_so': id_so, 'fundamenta_frac': fundamenta_frac, 'id_fundamenta_frac': id_fundamenta_frac, 'id_articulo': id_articulo},
					dataType: "html",
					beforeSend: function () {
