					...
					<?php } else { ?>
						...
						?>  id = "just<?php echo$data_frac->articulo . $data_frac->id_fraccion ?>">
							...
							<div class="input-field col s10">
								<textarea  name ="fundamenta_frac" id="<?php echo $data_frac->articulo . $data_frac->id_fraccion ?>" class="materialize-textarea justifica_so validate active"><?php echo $data_frac->just_so_oesp ?></textarea>
						...
						<div class="input-field col s2 center" >
							<a   class=' btn btn-floating btn-small  waves-effect waves-light edit_just_esp' href="#!" data-value="<?php echo $data_frac->id_fraccion ?>" id="save<?php echo $data_frac->articulo . $data_frac->id_fraccion ?>"><i class="material-icons">save</i></a>
						</div>
...
//				$('#' + id).val("");
		...
		$("body").on('click', ".edit_just_esp", function (event) {
...
//				$(this).attr("disabled", true);
				...
				$.ajax({
					async: true,
					type: "POST",
					url: base_url + "tablas_ap/edit_esp_fun_so",
					cache: false,
					data: {'id_frac': id_fraccion, 'ejercicio': ejercicio, 'id_cat_so': id_cat_so, 'id_so': id_so, 'fundamenta_frac': fundamenta_frac, 'id_fundamenta_frac': id_fundamenta_frac, 'aplica': aplica, 'id_articulo': id_articulo},
					dataType: "html",
					beforeSend: function () {
