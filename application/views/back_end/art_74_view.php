					...
					<?php } else { ?>
						<div style="display:none" id = "just<?php echo$data_frac->articulo . $data_frac->id_fraccion ?>">
							...
							<div class="input-field col s2" >
								<a  disabled class=' btn btn-floating btn-small  waves-effect waves-light save_just' href="#!" data-value="<?php echo $data_frac->id_fraccion ?>" id="save<?php echo $data_frac->articulo . $data_frac->id_fraccion ?>"><i class="material-icons">save</i></a>
							</div>
		...
		$("body").on('click', ".save_just", function (event) {
			...
			if (!this.checked) {
				...
				$.ajax({
					...
					cache: false,
					data: {'id_frac': id_fraccion, 'ejercicio': ejercicio, 'id_cat_so': id_cat_so, 'id_so': id_so, 'fundamenta_frac': fundamenta_frac, 'id_fundamenta_frac': id_fundamenta_frac},
					dataType: "html",
					beforeSend: function () {
